...
const Fridge = () => {
  ...
  useEffect(() => {
    const fetchFoods = async () => {
      ...
      const availableItems = data.filter((item) => {
        if (!item.expiryDate) return true; // keep if no expiry date
        return new Date(item.expiryDate) >= today; // keep if expiry date is in the future
      });
  ...
  return (
    <div className="">
      {/* bg-gradient-to-tr from-[#f3fff5] to-[#e5fbe0] */}
      <div className="max-w-7xl mx-auto   ">
        {/* border-green-200 dark:border-green-700 transition-colors duration-300 dark:bg-gray-900 px-6 py-12 rounded-2xl shadow-xl border */}
